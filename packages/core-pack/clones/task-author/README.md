...
**命名**：真正分叉的三个技能冠 `author-` 前缀（原名 `matt-verified-*`），与
`.claude/skills/` 里的上游技能**刻意不同名**——SDK 按 frontmatter `name` 发现技能，
同名会让分身会话加载到哪一份变成歧义。三个零改动拷贝保留原名：内容一致时同名无害。
...
warn；manifest 缺失（老安装首次迁移）→ 备份现有 `persona.md` + `skills/` 到
`seed.bak-<ts>/` 后全量接管。**孤儿清理**：本目录里删掉/改名的技能，其已安装的
旧副本若在 seed 之后未被手改 → 下次启动自动删除（连带剪空目录），确保退役的旧名
不再被 SDK 发现；手改过的旧副本属于用户，保留并 warn。详见
