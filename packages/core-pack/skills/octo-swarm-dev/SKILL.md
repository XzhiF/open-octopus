...
2. LLM 选择: haiku 模型从候选中选 2-5 专家 + 决定模式
...
**降级链**：`host.model ?? opus → sonnet → 拼接兜底 (degraded: true)`
...
- **Review 模式用 haiku** — 审查任务不需要强推理，haiku 足够且成本低
