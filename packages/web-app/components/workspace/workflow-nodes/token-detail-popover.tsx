      ...
      <PopoverContent className="w-56 p-3" side="right" align="start">
        <div className="space-y-2">
          ...
          {usages.map((u, i) => {
            ...
            <div key={`${u.model}-${i}`} className="space-y-1">
              <div className="text-xs font-medium border-b border-dashed border-border/50 pb-0.5">
                {u.model}
                {isRunning && (
                  <span className="inline-block h-1.5 w-1.5 rounded-full bg-pop-amber animate-pulse ml-1" />
                )}
              </div>
              <div className={cn("text-xs tabular-nums pl-1", isRunning ? "text-pop-amber" : "text-muted-foreground")}>
          ...
          <div className="border-t pt-1 space-y-0.5">
            <div className={cn("text-xs tabular-nums font-medium", isRunning ? "text-pop-amber" : "text-muted-foreground")}>
