// 04 · 联动下钻筛选（billing-report-3 ticket 04）· API 集成测试
...
// Hono app.request() 进程内真实路由栈 + 真实 SQLite；期望值 SQL 逐行独立复算（禁自推 —— Tautological 禁令）。
...
const PRICED = { cost_usd: 1, cost_native: 1, cost_currency: 'USD', price_status: 'priced' as const }
...
beforeAll(() => {
  ...
  billing.createPrice({ id: 'p-drill-m1', vendor: 'E2E_TEST_DV1', model_id: 'E2E_TEST_DM1', input_unit_price: 1, output_unit_price: 1, cache_write_unit_price: 0, cache_read_unit_price: 0, currency: 'USD' })
  billing.createPrice({ id: 'p-drill-m2', vendor: 'E2E_TEST_DV2', model_id: 'E2E_TEST_DM2', input_unit_price: 1, output_unit_price: 1, cache_write_unit_price: 0, cache_read_unit_price: 0, currency: 'USD' })
  ...
  dao.insertLlmCall(row('dr-1', { model: 'E2E_TEST_DM1', timestamp: T.d1, workspace_id: 'ws-drill-1', session_id: 'sess-drill-a', source_path: 'workflow', ...PRICED }))
  dao.insertLlmCall(row('dr-2', { model: 'E2E_TEST_DM2', timestamp: T.d2, workspace_id: 'ws-drill-1', session_id: 'sess-drill-b', source_path: 'interaction', ...PRICED }))
  dao.insertLlmCall(row('dr-3', { model: 'E2E_TEST_DM3', timestamp: T.d3, workspace_id: 'ws-other', session_id: 'sess-drill-a', source_path: 'harness', ...UNPRICED }))
  dao.insertLlmCall(row('dr-4', { model: 'E2E_TEST_DM1', timestamp: T.d4, workspace_id: null, session_id: null, source_path: 'global_chat', ...UNPRICED }))
  dao.insertLlmCall(row('dr-5', { model: 'E2E_TEST_DM2', timestamp: T.d5, workspace_id: 'ws-drill-1', session_id: 'sess-drill-a', source_path: 'interaction', ...PRICED }))
  // 非本票数据（同库其他行）不应被筛选命中 —— 时间/归属均不同
  dao.insertLlmCall(row('dr-x', { model: 'E2E_TEST_DM1', timestamp: T.d5, workspace_id: 'ws-noise', session_id: 'sess-noise', source_path: 'workflow', ...PRICED }))
})
...
async function get(params: string): Promise<{ status: number; body: { calls: LlmCallRow[]; total: number } }> {
...
describe('下钻筛选注入 → GET /calls（票04 验证步骤3）', () => {
  it('session 排行第 1 点击注入 session_id → 首行归属正确、行数 = SQL 直查', async () => {
    ...
    // unpriced 行仍计入数量筛选（KD21：下钻是行筛选不是费用筛选）
  ...
  it('厂商分布点击注入 vendor → 行数 = SQL（model→价行关联），未配价行不入', async () => {
    ...
    expect(body.total).toBe(3) // dr-1 + dr-4 + dr-x（DM1 全部行，含 unpriced/NULL 归属）
