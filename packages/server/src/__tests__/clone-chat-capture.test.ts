...
//
// billing-coverage-2 票02 —— 分身聊天（CloneRuntime）入账。
...
// cost 期望 = 价格 × 行内 token /1e6 手算（独立真相源）：
//   USD {3,15,3.75,0.3}: 1000×3 + 500×15 + 100×3.75 + 200×0.3 = 10935 → 0.010935。
//   副模型 {1,2,0.5,0.1}: 400×1 + 200×2 + 50×0.5 + 25×0.1 = 850 → 0.00085。
...
describe("分身聊天入账（票02 / US1）", () => {
  it("一轮对话 result chunk → 恰一行：source_path=clone_chat、session_id=会话、四类 token、cost 手算一致", async () => {
    ...
    expect(r).toMatchObject({
      ...
      cost_native: expect.closeTo(0.010935, 6),
      cost_currency: "USD",
    })
  })

  it("未配价模型 → 行仍入账，cost 三列 NULL + unpriced（KD4 不估算）", async () => {
    const sessionId = await createSession()
  ...
  it("多模型 modelUsages → 每模型一行、各自算价（per-call 粒度，不并成一坨）", async () => {
    const sessionId = await createSession()
    control.chunks = [resultChunk({
    ...
    expect(primary.cost_usd).toBeCloseTo(0.010935, 6)
    expect(secondary.cost_usd).toBeCloseTo(0.0008275, 6) // 400×1+200×2+50×0.5(cc)+25×0.1(cr) = 827.5 /1e6
