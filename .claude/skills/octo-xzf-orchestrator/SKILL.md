...
description: "[DEPRECATED] 线性委派器 — 已废弃，execution-loop 改为单 implementer 垂直切片执行"
...
> **⚠️ DEPRECATED** — 执行层已改为单 implementer 垂直切片模式。
> 每个 tracer bullet 由一个全栈 agent 直接实现，不再需要 orchestrator 协调多专家。
> 保留此 skill 仅供水平切片模式参考。
...
子代理返回纯净 JSON 对象（无 markdown 围栏、无前后文本）。解析规则：

1. 取子代理输出的**第一个 `{` 到最后一个 `}`** 之间的子串
2. `JSON.parse` 解析
3. IF 解析失败 → 视为子代理协议违规，按 failed 处理，failure_reason 记录 "返回格式无法解析"
4. IF 解析成功 → 读 `status` 字段：
   - `passed` → 继续下一个 task
   - `failed` → 读 `failure_reason` + `last_error` + `fix_attempts`，进入重试

⚠️ 不要依赖子代理输出固定字段顺序，不要用正则匹配特定字段。先 JSON.parse 再按 key 取值。
...
for each task in dependency_order:
  1. delegate_to_{role}-expert(task: |
     执行任务。请先 Read:
     - 任务: 04-specs/spec-NNN/{task-file}
     - 共识: 04-specs/spec-NNN/consensus.md
     - 环境: 04-specs/test-environment.md
...
delegate_to_backend-expert(task: |
  执行任务。请先 Read:
  - 任务: 04-specs/spec-001/task-1-2-auth-backend.md
  - 共识: 04-specs/spec-001/consensus.md
  - 环境: 04-specs/test-environment.md
...
1. delegate_to_test-expert(task: |
   执行 E2E 验证。请先 Read:
   - 测试路线: 04-specs/spec-001/spec-test.md
   - 环境: 04-specs/test-environment.md
...
## 失败报告
子代理返回 failed 且重试仍失败后，协调者写入 `06-reports/failure-{timestamp}.md`:
...
路径: `.octopus/xzf/{feature}/05-execution/spec-{NNN}-{name}/checkpoint.json`
