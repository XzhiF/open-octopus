...
description: Full pipeline orchestration skill. DAG-based development with user-chosen execution mode (concurrent matt-dev-runner sub-agents or main-agent quick) -> independent code review -> deploy -> optional E2E verification via matt-e2e-tester (user-gated via spec.md Execution Decisions) -> Git PR delivery. Use when a requirement is clarified and needs end-to-end development + deployment + verification + delivery.
---
...
Input: <artifacts.dir>/<feature-slug>/brief.md
    |
Phase 1: DAG Orchestration -> parse issues/, identify stages, implement per user-chosen mode (concurrent matt-dev-runner sub-agents | main-agent quick)
    |
Phase 2: Code Review -> independent sub-agent reviews diff (裁判 ≠ 球员)
    |
Phase 3: Deploy -> local dev only, skip CI/CD
    |
Phase 4: E2E Verification (optional — ask user) -> if user opts in: invoke matt-e2e-tester agent
    |
...
**Dispatch**: Main session orchestrates directly — no single matt-dev-runner agent. Instead, parse the DAG and execute per stage in the user-chosen **execution mode** (see 1.0).

### 1.0 Execution Mode (user-gated)

Read spec.md `## Execution Decisions` → "Ticket Execution Mode":
- **sub-agent concurrent (default)** — spawn one matt-dev-runner sub-agent per ticket, concurrent per stage (step 1.3a). Fits multi-ticket / cross-package DAGs.
- **main-agent quick** — the current main agent implements tickets **directly**, one at a time in stage order, following the same per-ticket flow (Read Context → Explore → Implement → Verify). Fits 1-2 small tickets where sub-agent spawn overhead outweighs isolation. **Integration gate (1.3c) and stage commit (1.3c.5) still apply — only the implementer changes.**
- **Missing from spec** → ask the user now, record the answer back into spec.md, then proceed.

**E2E ticket exclusion**: the final `NN-e2e-verification.md` ticket (always generated by requirement clarification) is **NOT executed in Phase 1** — it is consumed by Phase 4. If Phase 4 is skipped by user decision, set its `## Status` to `skip (user decision)`.
...
**a) Execute the stage's tickets** — sub-agent mode: spawn concurrent matt-dev-runner sub-agents (one per ticket, parallel Agent tool calls in a single message); main-agent quick mode: implement directly, one ticket at a time in number order. Sub-agent prompt:
```
...
## Phase 4: E2E Verification (optional — user-gated)

> **⚠️ USER GATE**: Phase 4 is user-gated. The answer is normally already collected at requirement-clarification exit — read spec.md `## Execution Decisions` → "E2E Verification":
> - Recorded as **run** → execute Phase 4 per below (spawn matt-e2e-tester).
> - Recorded as **skip** → apply the **Skip path** (below), then go to Phase 5.
> - **Missing from spec** → ask the user now, with a recommendation (run when spec has browser E2E ACs / user-facing flows changed; skip is reasonable for pure internal refactors or docs-only), record the answer back into spec.md, then act on it.
>
> **Never decide this yourself.** Run only on a user "run"; skip only on a user "skip".

> **⚠️ IF USER OPTS IN**: You MUST spawn the `matt-e2e-tester` sub-agent via the Agent tool.
> Do NOT substitute with unit tests. Do NOT self-execute E2E tests.
...
**Dispatch (user opted in)**: Invoke the `matt-e2e-tester` agent.
...
#### Skip path (user decided skip in Execution Decisions)

When the E2E decision is **skip**:
1. Do NOT spawn matt-e2e-tester; Artifact Gate does not apply
2. Set the final E2E ticket's `## Status` to `skip (user decision)`
3. Record Phase 4 in pipeline-report.md as `SKIPPED — user decision`
4. PR body E2E section: "Skipped by user decision"
5. Still update `<artifacts.dir>/index.md` status to `done`
6. Proceed directly to Phase 5

#### Phase 4 Artifact Gate (MANDATORY — run path only)
...
<!-- If skipped by user decision: replace table with "SKIPPED — user decision" -->
...
根据 4.2 收集的迭代上下文生成 PR body 文件 `pr-body.md`。If Phase 4 was skipped by user decision, every "E2E Verification" section below instead contains one line: `Skipped by user decision`.
...
1. **Phases run in order**: 1 → 2 → 3 → 4 → 5. Phase 4 is the only user-gated skippable phase — governed by spec.md `## Execution Decisions` (ask only if missing); the orchestrator must never skip or run it on its own. Phase 1 execution mode (sub-agent / main-agent quick) is likewise user-gated.
...
3. **Orchestrate, don't execute**: Phase 1 dispatches per user-chosen execution mode — sub-agent concurrent (default, spawn matt-dev-runner per ticket) or main-agent quick (main agent implements tickets directly, gates still enforced); Phase 2 主 Agent 直接调用 code-review skill（主 Agent 天然独立于实现代码）; in default mode main agent doesn't write code or run tests
