...
| 3 | agent-clone-system-refactor | 2026-07-24 | feat-builit-in-engines | done |
...
| 52 | phase-splitting-methodology | 2026-09-06 | octopus-feat-v4-direct-create-ui | in-progress (拆 Phase 方法论换锚：phase=完整故事/MVP 首 slice/功能票≥3 sanity/时间锚降票层，两段预算+不画雾+前提引用；grilling 七叉收敛 10 KD，walk-through 10 断点全收；ADR-0020。真机 dogfood 首航(6c0db77d 全局token计费)判据面 PASS，揪 4 工程根因已修:RC1a MoA硬编码→条件渲染/RC1b mingw-curl GBK乱码→curl 配方改 --data-binary @file(已恢复被毁 decisions)/RC2 漏写 phases→拆卡批准即写骨架/RC3 SpecPanel 产物区读 artifacts.json 看不见 .scratch→改名执行产物+指向 Phase 列;票05 执行面 AC3 待用户续会话补写 phases) |
| 53 | draft-artifact-visibility | 2026-09-06 | octopus-feat-v4-direct-create-ui | done (起草面产物可见性三痛点收口：C磁盘直扫草稿批次区+Phase行内展开+R1对话事件刷新+manifest降位，点线图三叉钉死后落地；server GET :id/batch-tree＋web 75→77用例全绿＋playwright穿线＋活样本6c0db77d眼验；K3纠错resources/authoring_resources非死键只滤空数组；task家族4红经stash对照全为既有/环境红；R1真LLM实弹按成本口径SKIP) |
| 54 | task-scheduler-decouple | 2026-09-09 | feat/scheduler-decouple | 票01-06 全落地·待 provider 验收 (任务⇄调度器彻底切开：调度器职责回归 workflow/agent 两类作业 + 新增第三类 job(TS handler 注册表)，任务启动/状态推进/孤儿回收交给一个内置 job=task-lifecycle；触发定义上移 tasks.trigger_mode/trigger_at/next_fire_at，一次任务运行就是 executions 那一行(+task_id 直连，废经 schedules 的 join 桥)，「一任务一实例」从 ~10 处守卫降为 ux_exec_task_active 根执行 partial UNIQUE(NOT IN 终态 fail-closed)；不新增表(初稿 cron_jobs+scheduler_runs 双表方案已否决删除)、node-cron 保留、任务域禁出现 schedules(门禁测试)；ADR-0021。票03 原子翻转(job 实装/信封退役/v42 删列+清存量幽灵信封/门禁测试)、票04 子单元=child executions + 两条 resume 归一 + 丢唤醒 tick 自愈、票05 读模型收进 shared(Task 自带 trigger_*/execution badge，红行带 error_summary+reason，fan-out 带 children)+ 调度页回归作业视图(job 类型通 API、内置 job 可见/可暂停/不可删/不可编辑、上次触发带耗时)、票05 真机揪出并修 4 个"全绿仓库里躺着"的洞(claim→start lease 交接、内置 job 可被删死、create() 丢 task_id、v42 只删列留幽灵作业)；票06 三个 task-domain spec 改查任务执行列表 + 新增 task-trigger-loop.spec(零信封→排期→到点→闩锁→中止 五段) + 中止/退役路径补发 task_execution(带 reason)。基线红(实测)：server 35/8 文件、shared 4、engine 4、cli 0、web 8/4 文件——票05 后 server 少的 2 条正是 scheduler-routes 那两条 G7 遗留。真机复验两轮见 spec §8b/§8c；**仍待有 provider+真 repo 的机器**：手测② 一轮真跑转 running、③ 连续两起、④ abort 后槽位被下一个排队任务接走、⑤ 浏览器走查、⑥ 重启后 RecoveryManager 与内置 job 共存 |
| 55 | task-exec-tree | 2026-09-15 | octopus-feat-task-exec-tree | code-done·待重启+数据修复 (任务执行树：v4 轮次自此挂在上一轮下，外层=一棵树；schema v44 把 ux_exec_task_active 从 roots-only 放宽到实例谓词 `(parent='0' OR phase_index IS NOT NULL)`，arm-vs-arm 判据换轨 phase 标记=实例、无标记有父=composite 臂——launchQueued/finalizeLaunch/deriveView/findTaskRound/listTaskChildRuns/dashboard CTE/token 统计六路全收口；armTask 带 same-ws 守卫(ws 重建另起新树)；$parent 继承只认真子流(链式轮启动逐字节同旧)；chain auto_execute 跳过 task 行(任务树归 lifecycle job)；v44 迁移 fail-closed 防双活炸启动；task-lifecycle +9 测全绿，server 全量=基线红35∅新增；fix-data.sql 幂等只动终态散根，★必须重启后跑(在飞行改链=旧 finalize 误走子单元分支丢 collect/待验收)，全库唯一待链行=a04aab5a) |
