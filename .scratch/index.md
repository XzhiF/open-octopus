...
| 54 | task-scheduler-decouple | 2026-09-09 | feat/scheduler-decouple | in-progress (任务⇄调度器彻底切开：调度器职责回归 workflow/agent 两类作业 + 新增第三类 job(TS handler 注册表)，任务启动/状态推进/孤儿回收交给一个内置 job=task-lifecycle；触发定义上移 tasks.trigger_mode/trigger_at/next_fire_at，一次任务运行就是 executions 那一行(+task_id 直连，废经 schedules 的 join 桥)，「一任务一实例」从 ~10 处守卫降为 ux_exec_task_active 根执行 partial UNIQUE(NOT IN 终态 fail-closed)；不新增表(初稿 cron_jobs+scheduler_runs 双表方案已否决删除)、node-cron 保留、任务域禁出现 schedules(门禁测试)；ADR-0021；票01-05 已落：票03 原子翻转(job 实装/信封退役/v42 删列/门禁测试)、票04 子单元=child executions + 两条 resume 归一 + 丢唤醒 tick 自愈、票05 读模型收进 shared(Task 自带 trigger_*/execution badge，红行带 error_summary，fan-out 带 children)+ 调度页回归作业视图(job 类型通 API)。基线红：server 36/9 文件、shared 4、engine 4、web 9/4-5 文件(均既有/环境红)。余票06：6 个 e2e spec 改查任务执行列表 + §8 手测清单) |
