...
## What to build
SG1 origin 迁移：`scheduler-engine.ts:379-391`(failed-提升门控→origin_type='task')+`:448`(认领过滤→origin_type IN task/manual/api)+`task-dispatch-service.ts:314`(子创建设 origin_type/origin_role/origin_id，替 trigger_source='requirement')+`TaskDispatchService.dispatchChildSchedule` 重写+`TaskDispatchPort`+origin_role。审计其它 trigger_source 分支。SG5 `materializeTaskSpecToConfig` export+输出 drop task_spec+复合注 subunit_count。SG9 isComposite N≥2（1-subunit 走 workflow_chain）。SG10 child running SSE emit。SG16 barrel re-export TaskDispatchService。SG12 孤儿 reaper（定时扫删 schedules WHERE origin_type='task' AND origin_id NOT IN active tasks）。R-INT cascade-reap on task delete。
