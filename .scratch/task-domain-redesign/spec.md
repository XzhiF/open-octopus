...
| SG6 | authoring_resources 每 turn 注入 | **SPIKE S2 已验**：Mechanism B=task-author clone 每 turn fresh session（`providerSessionId=null`，`assembleContext` 读 `authoring_resources[]` SKILL.md 注 systemPrompt；DB 历史 prepend 到 user msg 续上下文）。`getSystemPrompt` 闭包仅 creation-time、`_rebuildSystemPrompt` 非 per-turn；`before_agent_start` hook 可用但需先修 resume 路径。Mechanism B 不改 v2-D8 |
...
- **SPIKE S2（SG6）— RESOLVED**：authoring_resources 每 turn 注入走 Mechanism B（task-author fresh session per turn + assembleContext 注入 + DB 历史 preamble）；不改 v2-D8。**Latent bug（pre-existing）**：Pi SDK resume 路径 broken（`findSession` 返 SessionManager 非 AgentSession，turn 2+ TypeError/失历史）— v2 task-author 经 fresh-session 绕开；out of v2 scope，建议另起 ticket 修。
