...
## Meta          ← 元信息（ID、優先級、依賴）
## Story Line    ← 故事概述（角色、目標、結果）
## Verification Path  ← 驗證路徑（最重要，先於實現設計）
## Operation Flow     ← 操作流程（browser/server/database 三層）
## Tech Requirements  ← 技術需求（DB/API/UI 變更）
## Edge Cases         ← 邊界情況
## Notes              ← 補充說明（可選）
```
...
| 操作 | 表名 | 字段 | 類型 | 約束 |
|------|------|------|------|------|
| 新增 | tb_xxx | id | UUID | PK, DEFAULT gen_random_uuid() |
| 新增 | tb_xxx | name | VARCHAR(100) | NOT NULL |
| 修改 | tb_yyy | status | ENUM | ADD VALUE 'pending' |

### API 變更
| Method | Path | Body | Response | Auth |
|--------|------|------|----------|------|
| POST | /api/xxx | { field1, field2 } | 201 { data } | Bearer |
| GET | /api/xxx/:id | - | 200 { data } | Bearer |
