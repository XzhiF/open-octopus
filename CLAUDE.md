...
Y Combinator CEO Garry Tan 的 AI 工程技能包，提供虚拟工程团队角色。所有 skill 全局安装在 `~/.claude/skills/gstack/`。

## 核心 Skills

**产品与规划**
- **office-hours**: YC 式产品拷问，6 个 forcing questions 验证需求真实性
- **plan-ceo-review**: CEO 视角审查方案，4 种 scope 模式（扩张/精选/保持/精简）
- **plan-eng-review**: 工程经理视角，锁架构、数据流、边界条件、测试覆盖
- **plan-design-review**: 设计师视角，评分每个设计维度 0-10
- **autoplan**: 自动串联 CEO/设计/工程三道审查，一次出完整方案

**开发与审查**
- **review**: PR 预合并审查，检测 SQL 安全、LLM 信任边界、条件副作用
- **cso**: 安全官模式，OWASP Top 10 + STRIDE 威胁建模 + 密钥考古
- **investigate**: 系统化 debug 四阶段（调查→分析→假设→实现），铁律：无根因不修复
- **codex**: OpenAI Codex 第二意见（review/challenge/consult 三模式）

**QA 与部署**
- **browse**: 无头浏览器 daemon（~100ms/命令），snapshot/click/fill/screenshot
- **qa**: 用真浏览器做 E2E 测试
- **benchmark**: 性能基线 + PR 级回归检测
- **canary**: 部署后金丝雀监控
- **ship**: 自动合 PR + 部署
- **land-and-deploy**: 落地并部署

**设计**
- **design-consultation**: 生成 DESIGN.md（字体+配色+布局系统）
- **design-shotgun**: 多方案视觉对比
- **design-review**: 视觉 QA（间距、层级、AI slop 检测）

**辅助**
- **careful**: 破坏性命令拦截（rm -rf、DROP TABLE、force-push）
- **guard**: 安全护栏
- **retro**: 工程复盘
- **learn**: 从会话提取可复用模式
